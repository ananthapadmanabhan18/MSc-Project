...
void Clusterizer(Int_t startFolder, Int_t endFolder, Int_t startEvent, Int_t endEvent) {
   
#include "ClusterizerDataSample.C"
#include "ClusterizerParameters.C"
    // _TAG_CONFIG_ Here will be an clusterizerDataSample information with clusterizer parameters following
    
    
    AliFOCALClusterizerv2 *fFOCALCluster = new AliFOCALClusterizerv2();
...
//	  fFOCALCluster->SetClusteringEnergyThreshold(0.004); //40MeV 
	  fFOCALCluster->SetDistance(distance);  // set the range you will look to make a cluster
	  ...
	  AliFOCALGeometry * geometry = fFOCALCluster->GetGeometry();
    ...
    for(Int_t nfolder = startFolder; nfolder <= endFolder; nfolder++) {
        ...
        TFile * outputFile = fFOCALCluster->CreateOutputFile(Form("%s/clusters_%s_%s_%i.root",clusteringOutputFileDir,dataSampleTag,clusteringParametersTag,nfolder));
        
        if (clusteringSaveImages) {
          char createSaveDir[100];
          sprintf(createSaveDir,"mkdir -p %s/%02i",clusteringImagesDir,nfolder);
          gSystem->Exec(createSaveDir);
        }
        
        // _TAG_EVENTLOADER_ Here will be loader depending whether loading digits or hits            
#include "ClusterizerHitLoader.C"
            ...
            for (Int_t i = 0; i < nClusters; i++) {
              ...
              if (cluster->Segment() != FINE_SEEDS[0])
                continue;
            ...
            for (Int_t i = 0; i < nClusters; i++) {
              ...
              if (cluster->Segment() != COARSE_SEEDS[0])
                continue;
...
//            finalClusterMap->ResetMap(); // Not used here...
            ...
            for (Int_t seedSegment = 0; seedSegment < sizeof(ALL_SEEDS)/sizeof(ALL_SEEDS[0]); seedSegment++) 
            {
              ...
              for (Int_t c = 0; c < nSubClusters; c++) 
              {
                ...
                else 
                {
                  ...
                  if (!found && fFOCALCluster->GetDebugMode())
                    cout << Form("WARNING cluster at [%.2f,%.2f] of segment %i not registered in the map, no space left in vicinity!",x/energy,y/energy,finalSegment);
                } // end else register
	    ...
	    if (fFOCALCluster->GetDebugMode())
	      ...
	      newCluster->SetIsoEnergyR4(eHCAL);
            ...
            if (clusteringSaveImages)
              fFOCALCluster->SaveImage(Form("%s/%02i/event_%02i",clusteringImagesDir,nfolder,ievt));
